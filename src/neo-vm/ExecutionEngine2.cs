...
namespace Neo.VM
{
    ...
    /// </summary>
    public class ExecutionEngine2 : IDisposable
    {
        private VMState state = VMState.BREAK;
        private bool isJumping = false;
        //private ObjectPool<Integer> pool = new ObjectPool<Integer>(()=>new Integer(0));
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ExecutionEngine2"/> class.
        /// </summary>
        public ExecutionEngine2() : this(new ReferenceCounter(), ExecutionEngineLimits.Default)
        {
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="ExecutionEngine2"/> class with the specified <see cref="VM.ReferenceCounter"/> and <see cref="ExecutionEngine2Limits"/>.
        /// </summary>
        /// <param name="referenceCounter">The reference counter to be used.</param>
        /// <param name="limits">Restrictions on the VM.</param>
        protected ExecutionEngine2(ReferenceCounter referenceCounter, ExecutionEngineLimits limits)
        {
            this.Limits = limits;
        ...
        private void ExecuteInstruction()
        {
            ...
            switch (instruction.OpCode)
            {
                ...
                case OpCode.PUSHINT256:
                    {
                        Push(PoolManager.Instance.Reuse(new BigInteger(instruction.Operand.Span)));
                        break;
                    }
                case OpCode.PUSHA:
                    {
                ...
                case OpCode.PUSH16:
                    {
                        Push(PoolManager.Instance.Reuse((int)instruction.OpCode - (int)OpCode.PUSH0));
                        break;
                    }
                ...
                case OpCode.INVERT:
                    {
                        ...
                        Push(PoolManager.Instance.Reuse(~x));
                        break;
                    }
                case OpCode.AND:
                    {
                        var x2 = Pop().GetInteger();
                        var x1 = Pop().GetInteger();
                        Push(PoolManager.Instance.Reuse(x1 & x2));
                        break;
                    }
                case OpCode.OR:
                    {
                ...
                case OpCode.EQUAL:
                    {
                        StackItem x2 = Pop();
                        StackItem x1 = Pop();
                        Push(PoolManager.Instance.Reuse(x1.Equals(x2, Limits)));
                        break;
                    }
                ...
                case OpCode.SIGN:
                    {
                        var x = Pop().GetInteger();
                        Push(PoolManager.Instance.Reuse(x.Sign));
                        break;
                    }
                case OpCode.ABS:
                    {
                        var x = Pop().GetInteger();
                        Push(PoolManager.Instance.Reuse(BigInteger.Abs(x)));
                        break;
                    }
                case OpCode.NEGATE:
                    {
                ...
                case OpCode.INC:
                    {
                        var x = Pop().GetInteger();
                        Push(PoolManager.Instance.Reuse(x + 1));
                        break;
                    }
                case OpCode.DEC:
                    {
                ...
                case OpCode.POW:
                    {
                        ...
                        Push(PoolManager.Instance.Reuse(BigInteger.Pow(value, exponent)));
                        break;
                    }
                case OpCode.SQRT:
                    {
                        Push(PoolManager.Instance.Reuse(Pop().GetInteger().Sqrt()));
                        break;
                    }
                case OpCode.SHL:
                    {
                        ...
                        if (shift == 0) break;
                        var x = Pop().GetInteger();
                        Push(PoolManager.Instance.Reuse(x << shift));
                        break;
                    }
                case OpCode.SHR:
                    {
                ...
                case OpCode.NZ:
                    {
                        var x = Pop().GetInteger();
                        Push(PoolManager.Instance.Reuse(!x.IsZero));
                        break;
                    }
                ...
                case OpCode.LT:
                    {
                        ...
                        if (x1.IsNull || x2.IsNull)
                            Push(PoolManager.Instance.Reuse(false));
                        else
                            Push(PoolManager.Instance.Reuse(x1.GetInteger() < x2.GetInteger()));
                        break;
                    }
                case OpCode.LE:
                    {
                ...
                case OpCode.MIN:
                    {
                        var x2 = Pop().GetInteger();
                        var x1 = Pop().GetInteger();
                        Push(PoolManager.Instance.Reuse(BigInteger.Min(x1, x2)));
                        break;
                    }
                case OpCode.MAX:
                    {
                        var x2 = Pop().GetInteger();
                        var x1 = Pop().GetInteger();
                        Push(PoolManager.Instance.Reuse(BigInteger.Max(x1, x2)));
                        break;
                    }
                case OpCode.WITHIN:
                    {
                        ...
                        var x = Pop().GetInteger();
                        Push(PoolManager.Instance.Reuse(a <= x && x < b));
                        break;
                    }
                ...
                case OpCode.UNPACK:
                    {
                        ...
                        for (int i = array.Count - 1; i >= 0; i--)
                            Push(array[i]);
                        Push(PoolManager.Instance.Reuse((BigInteger)array.Count));
                        break;
                    }
                ...
                case OpCode.SIZE:
                    {
                        ...
                        switch (x)
                        {
                            case CompoundType compound:
                                Push(PoolManager.Instance.Reuse((BigInteger)compound.Count));
                                break;
                            case PrimitiveType primitive:
                                Push(PoolManager.Instance.Reuse((BigInteger)primitive.Size));
                                break;
                            case Buffer buffer:
                                Push(PoolManager.Instance.Reuse((BigInteger)buffer.Size));
                                break;
                            default:
                            ...
                            case Buffer buffer:
                                {
                                    ...
                                    if (index < 0)
                                        throw new InvalidOperationException($"The negative value {index} is invalid for OpCode.{instruction.OpCode}.");
                                    Push(PoolManager.Instance.Reuse(index < buffer.Size));
                                    break;
                                }
                            case ByteString array:
                                {
                                    ...
                                    if (index < 0)
                                        throw new InvalidOperationException($"The negative value {index} is invalid for OpCode.{instruction.OpCode}.");
                                    Push(PoolManager.Instance.Reuse(index < array.Size));
                                    break;
                                }
                            default:
                                throw new InvalidOperationException($"Invalid type for {instruction.OpCode}: {x.Type}");
                ...
                case OpCode.ISTYPE:
                    {
                        ...
                        Push(PoolManager.Instance.Reuse(x.Type == type));
                        break;
                    }
                case OpCode.CONVERT:
                    {
        ...
        public StackItem Pop()
        {
            ...
            var a = CurrentContext!.EvaluationStack.Pop();
        ...
        {
            var a = CurrentContext!.EvaluationStack.Pop<T>();
            PoolManager.Instance.Collect(a);
            return a;
        }
